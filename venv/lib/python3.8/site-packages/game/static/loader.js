...
(function (Promise) {
        ...
        //  See also https://www.dosbox.com/wiki/MOUNT
         if (files[i].drive_type === 'floppy') {
          ...
          args.push('-c', 'mount '+ files[i].drive +' /emulator'+ files[i].mountpoint + ' -t cdrom');
           ...
           //default behavior is mount folder as drive letter
   ...
   */
   ...
   function Emulator(canvas, callbacks, loadFiles) {
     ...
     var loading = false;
     var defaultSplashColors = { foreground: '#6affbf',
                                 background: 'black',
                                 failure: 'yellow' };
     var splash = { loading_text: "Powered by leesoar.com",
                    spinning: true,
       ...
       loading.then(function (_game_data) {
              ...
              .then(function (game_files) {
                    ...
                    function () {
                      ...
                      splash.setTitle("Failed to load game data!");
                      splash.failed_loading = true;
                    })
              .then(function () {
                      if (!game_data || splash.failed_loading) {
